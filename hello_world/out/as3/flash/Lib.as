package flash {
	...
	public class Lib {
		...
		static protected function traceToConsole(v : *,inf : * = null) : void {
			var type : String;
			if(inf != null && inf.customParams != null) type = inf.customParams[0];
			else type = null;
			if(type != "warn" && type != "info" && type != "debug" && type != "error") if(inf == null) type = "error";
			else type = "log";
			var str : String;
			if(inf == null) str = "";
			else str = inf.fileName + ":" + inf.lineNumber + " : ";
			try {
				str += Std.string(v);
			}
			catch( e : * ){
